                    ...
                    if result['success']:
                        ...
                        if 'parsing_res_list' in data and data['parsing_res_list']:
                            for item in data['parsing_res_list']:
                                ...
                                # 提取PaddleOCR输出中的content内容
                                extracted_content = ''
                                
                                if isinstance(text_content, dict):
                                    # 如果是字典，直接取content字段
                                    extracted_content = text_content.get('content', str(text_content))
                                elif isinstance(text_content, str):
                                    # 如果是字符串，尝试用正则表达式提取content
                                    import re
                                    content_match = re.search(r'content:\s*(.+?)(?=\n|\r\n|#################)', text_content, re.DOTALL)
                                    if content_match:
                                        extracted_content = content_match.group(1).strip()
                                    else:
                                        # 如果正则不匹配，尝试直接查找content行
                                        lines = text_content.split('\n')
                                        for line in lines:
                                            line = line.strip()
                                            if 'content:' in line.lower():
                                                extracted_content = line.split('content:')[-1].strip()
                                                break
                                        if not extracted_content:
                                            extracted_content = text_content  # 如果都没找到，使用原始内容
                                else:
                                    extracted_content = str(text_content)
                                print(f"#########type:{extracted_content}")
                                
                                f.write(f"图像名称: {result['image_name']}, 坐标: {result['bbox_coords']}, 识别文本: {extracted_content}\n")
                        else:
                            # 从数据中提取content:后的实际内容
                            import re
                            text_str = str(data)
                            # 匹配content:后面的内容直到换行或###分隔符
                            content_match = re.search(r'content:\s*(.+?)(?=\n|\r\n|#################)', text_str, re.DOTALL)
                            if content_match:
                                extracted_content = content_match.group(1).strip()
                                f.write(f"图像名称: {result['image_name']}, 坐标: {result['bbox_coords']}, 识别文本: {extracted_content}\n")
                                ...
                                for line in lines:
                                    ...
                                    if line.lower().startswith('content:'):
                                        extracted_content = line[8:].strip()  # 去掉'content:'前缀
                                        f.write(f"图像名称: {result['image_name']}, 坐标: {result['bbox_coords']}, 识别文本: {extracted_content}\n")
                                        break
                                else:
                                    # 如果都没有找到，保存原始内容
                                ...
                                for item in data['parsing_res_list']:
                                    ...
                                    if content_match:
                                        ...
                                        for line in lines:
                                            ...
                                            print(f"#########type:{text_content}")
                            else:
                                # 从数据中提取content字段的实际内容
                                # 检查是否是字典格式的输出（PaddleOCR的结构化输出）
                                if isinstance(data, dict):
                                    # 检查是否有parsing_res_list，如果有，取第一个项目的content
                                    if 'parsing_res_list' in data and data['parsing_res_list']:
                                        first_item = data['parsing_res_list'][0]
                                        if isinstance(first_item, dict) and 'content' in first_item:
                                            extracted_content = first_item['content']
                                            f.write(f"图像名称: {result['image_name']}, 坐标: {result['bbox_coords']}, 识别文本: {extracted_content}\n")
                                            print(f"#########type:{extracted_content}")
                                        else:
                                            # 如果parsing_res_list中的项目没有content字段，尝试其他方式
                                            f.write(f"图像名称: {result['image_name']}, 坐标: {result['bbox_coords']}, 识别文本: {data}\n")
                                            print(f"#########type:{data}")

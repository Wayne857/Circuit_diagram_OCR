...
def zhang_suen_skeletonize(binary):
    """Zhang-Suen 骨架提取算法"""
    img = binary.copy()
    h, w = img.shape
...
def merge_close_points(points, threshold=3):
    """合并距离小于阈值的特征点（去重）"""
    merged = []
    ...
    for i in range(len(points)):
        ...
        for j in range(i+1, len(points)):
            if not used[j] and distance(curr_point, points[j][1]) < threshold:
...
def merge_close_segments(segments, threshold=5):
    ...
    for i in range(len(segments)):
        ...
        while True:
            ...
            for j in range(len(segments)):
                ...
                if (distance(end_i, start_j) < threshold or 
                    distance(end_i, end_j) < threshold or
                    distance(start_i, start_j) < threshold or
                    distance(start_i, end_j) < threshold):
                    
                    # 合并两条线段的路径
                    if distance(end_i, start_j) < threshold:
                        new_path = path_i + path_j
                        new_start = start_i
                        new_end = end_j
                    elif distance(end_i, end_j) < threshold:
                        new_path = path_i + path_j[::-1]
                        new_start = start_i
                        new_end = start_j
                    elif distance(start_i, start_j) < threshold:
...
def detect_feature_points(skeleton):
    ...
    # 合并距离极近的特征点
    feature_points = merge_close_points(feature_points, threshold=3)
    return feature_points

def trace_wire_segments(skeleton, feature_points):
    """追踪导线线段（优化BFS，允许微小偏移）"""
    h, w = skeleton.shape
    visited = set()
    segments = []
    ...
    segments = merge_close_segments(segments, threshold=5)
    return segments

def build_connection_graph(segments):
    """构建连接关系图"""
    G = nx.Graph()
...
def visualize_results(img, skeleton, feature_points, segments):
    """可视化结果（新增导线序号标注）"""
    ...
    plt.title('Skeleton (After Repair)')
    plt.axis('off')
    ...
    for point_type, (x, y) in feature_points:
        color = 'red' if point_type == 'endpoint' else 'blue'
        plt.scatter(x, y, c=color, s=20, label=point_type if point_type not in plt.gca().get_legend_handles_labels()[1] else "")
    
    # 2. 绘制线段并添加序号
    for idx, (start, end, path) in enumerate(segments, 1):  # 序号从1开始
        # 绘制导线线段
        x_coords = [p[0] for p in path]
        y_coords = [p[1] for p in path]
        plt.plot(x_coords, y_coords, c='green', linewidth=1)
...
if __name__ == "__main__":
    # 替换为你的图片路径
    image_path = "runs/run12/segmented_out/complex_without_segments.jpg"
    
    try:
        # 1. 预处理（含闭运算补断裂）
        img, binary, skeleton = preprocess_image(image_path)
        
        # 2. 检测特征点（含去重）
        feature_points = detect_feature_points(skeleton)
        print(f"去重后特征点：{len(feature_points)}个（端点+交点）")
        
        # 3. 追踪线段（含合并）
        segments = trace_wire_segments(skeleton, feature_points)
        print(f"合并后导线线段：{len(segments)}条")
        
        # 4. 构建连接图
        connection_graph = build_connection_graph(segments)
        print(f"连接图节点数：{connection_graph.number_of_nodes()}，边数：{connection_graph.number_of_edges()}")
        
        # 5. 可视化（带序号）
        visualize_results(img, skeleton, feature_points, segments)

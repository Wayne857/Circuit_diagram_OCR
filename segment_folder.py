...
def segment_folder(folder_path, model_path, output_dir, conf=0.5):
    """
    对整个文件夹中的图像进行分割
    """
...
def main():
    parser = argparse.ArgumentParser(description='Segment Folder with YOLOv8')
    parser.add_argument('--image', '-i', type=str, help='Input image path')
    parser.add_argument('--folder', '-f', type=str, help='Input folder path')
    ...
    if args.image:
        ...
        # 处理整个文件夹
        segment_folder(args.folder, args.model, args.output, args.conf)
    else:

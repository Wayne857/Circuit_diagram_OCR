...
def predict_and_save_detailed_segmentation(image_path, model_path, output_dir, conf=0.5):
    """
    使用指定模型进行分割预测，并保存详细结果
...
def predict_folder_detailed(folder_path, model_path, output_dir, conf=0.5):
    """
    对整个文件夹中的图像进行分割预测
    """
    ...
    for i, image_file in enumerate(image_files):
        ...
        predict_and_save_detailed_segmentation(
            str(image_file), 
            model_path, 
...
def main():
    parser = argparse.ArgumentParser(description='Detailed Segmentation with YOLOv8')
    parser.add_argument('--image', '-i', type=str, help='Single image path')
    parser.add_argument('--folder', '-f', type=str, help='Image folder path')
